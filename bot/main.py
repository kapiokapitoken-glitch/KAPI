...
# =========================
# ENV – senin değerlerine göre
# =========================
TELEGRAM_BOT_TOKEN = os.environ["TELEGRAM_BOT_TOKEN"]  # 8383... (DO'da Secret)
DATABASE_URL       = os.environ.get("DATABASE_URL")    # asyncpg URI (tek satır)
...
async def serve_index():
    ...
    return JSONResponse({"ok": True, "hint": "index.html bulunamadı"}, status_code=200)
...
async def ensure_schema() -> None:
    """scores tablosunu oluştur + index."""
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ /start → Oyunu Başlat butonu """
    kb = [[InlineKeyboardButton("Oyunu Başlat", url=PUBLIC_GAME_URL)]]
    await update.message.reply_text(
        "KAPI RUN’a hoş geldin! 🎮\nButona bas ve oyna. Skorların otomatik kaydedilecek.",
...
async def cmd_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ /top → En iyi 200 skor """
    if engine is None:
        await update.message.reply_text("Leaderboard şu an kullanılamıyor (DB yok).")
        return
    async with engine.connect() as conn:
        ...
        rows = list(res)
    if not rows:
        await update.message.reply_text("Henüz skor yok. İlk skor senin olsun! 🧣")
        return
    lines = []
...
async def telegram_webhook(request: Request):
    """Telegram webhook giriş noktası."""
...
async def on_startup():
    ...
    if DATABASE_URL:
        ...
        print("WARNING: DATABASE_URL not set; leaderboard/score API çalışmayacak.")
